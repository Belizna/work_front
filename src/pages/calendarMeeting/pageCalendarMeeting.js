...
const PageCalendarMeeting = () => {
  const [countSave, setCountSave] = useState(0);

  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL}/meetingCalendar`)
      .then((res) => setData(res.data.meetingCalendar))
  ...
  const handleDelete = async (record) => {
    ...
    await axios.delete(`${process.env.REACT_APP_API_URL}/meetingCalendar/delete/${record._id}`)
    setData(newData);
  };
  ...
  const edit = (record) => {
    form.setFieldsValue({
      meetingCalendar_date: (dayjs.utc(record.meetingCalendar_date, dateFormat)),
      meetingCalendar_agenda: record.meetingCalendar_agenda,
...
export default PageCalendarMeeting;